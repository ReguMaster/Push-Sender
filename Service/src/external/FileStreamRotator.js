...
var _checkDailyAndTest = function( freqType )
{
    switch ( freqType )
    {
        ...
        case 'daily':
            return {
                type: freqType, digit: undefined
            };
            break;
        case 'test':
            return {
                type: freqType, digit: 0
            };
    }
    return false;
}
...
FileStreamRotator.getDate = function( format, date_format, utc )
{
    ...
    let currentMoment = utc ? moment.utc( ) : moment( )
        .local( )
    if ( format && staticFrequency.indexOf( format.type ) !== -1 )
    {
        switch ( format.type )
        {
            case 'm':
                ...
                return currentMoment.minutes( minute )
                    .format( date_format );
                break;
            case 'h':
                var hour = Math.floor( currentMoment.hour( ) / format.digit ) * format.digit;
                return currentMoment.hour( hour )
                    .format( date_format );
                break;
            case 'daily':
            case 'custom':
...
FileStreamRotator.setAuditLog = function( max_logs, audit_file, log_file )
{
    ...
    if ( max_logs )
    {
        var use_days = max_logs.toString( )
            .substr( -1 );
        var _num = max_logs.toString( )
            .match( /^(\d+)/ );
...
{
    if ( file.hash === crypto.createHash( 'md5' )
...
FileStreamRotator.getStream = function( options )
{
    ...
    if ( typeof filename === "function" )
        logfile = filename( curDate ? curDate : self.getDate( null, dateFormat, options.utc ) );
        ...
        stream.write = ( function( str, encoding )
            {
                ...
                if ( newDate != curDate || ( fileSize && curSize > fileSize ) )
                {
                    ...
                    // *NOTE: CUSTOM
                    if ( typeof filename === "function" )
                        newLogfile = filename( curDate ? curDate : self.getDate( null, dateFormat, options.utc ) );
                    ...
                    if ( self.verbose )
                    {
                        console.log( new Date( ), require( 'util' )
                            .format( "[FileStreamRotator] Changing logs from %s to %s", logfile, newLogfile ) );
                    }
                    curDate = newDate;
                    oldFile = logfile;
                    logfile = newLogfile;
